...
### Applying Typing to Settings Retrieval

In our settings system, we can use generics to define the types of our settings. This will allow us to define the types of our settings in a single place, and have the IDE know what the types are when we access them. We can imagine a function that uses generics, but the actual return type is calculated _based on the requested setting_.

```typescript
const get = <T extends TypeName>(setting: T): ObjectType<T> => {
  // Get and return the setting from the DB
}
```

This function uses the generic `T` to set the return type of the function. We need to define `TypeName` (a type consisting of every setting name) and `ObjectType` (a type generic that can associate the setting name with the setting return type) to make this work:

```typescript
// Specify the names of all possible settings
type TypeName = 'my.setting.1' | 'my.setting.2' | 'my.setting.3';

// Assign each setting a return type
export type ObjectType<T extends TypeName> = 
  T extends 'my.setting.1' ? string :
  T extends 'my.setting.2' ? number :
  T extends 'my.setting.3' ? boolean :
  string;

```

Now TypeScript knows what settings are valid inputs, and what the return type will be:

```typescript
// IDE knows that s1 is a string, and can provide autocompletion
const s1 = settings.get('my.setting.1');
```

### Retrieving Many Settings

Our current method is great for getting a single setting, but will be very inefficient for getting multiple settings:

```typescript
const s1 = settings.get('my.setting.1'); // DB interaction
const s2 = settings.get('my.setting.2'); // DB interaction
const s3 = settings.get('my.setting.3'); // DB interaction
```

We want to be able to get all of these settings in _one_ DB call. Lets make another getter function:

```typescript
const getMany = <T extends TypeName>(...settings: T[]): { [K in FilterSettingsMany<TypeName, T>]: ObjectType<K> } => {
  // Get and return the settings from the DB
}
```
